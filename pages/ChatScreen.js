...
const ChatScreen = ({ navigation, route }) => {
  ...
  const fetchChatMessages = async (roomId) => {
    try {
      const response = await fetch(
        `https://rapport-backend.onrender.com/chat/messages/${roomId}`
      );
      ...
      if (data.messages && data.messages.length > 0) {
        data.messages.forEach(message => {
          const formattedMessage = {
            ...
            senderName: message.senderName || (message.senderId === userId ? 
              (profileData?.firstName && profileData?.lastName ? 
                `${profileData.firstName} ${profileData.lastName}` : "You") : 
              contact.name),
            senderAvatar: message.senderId === userId ? 
              (profileImage || "https://i.pravatar.cc/100?img=11") : 
              (contact.avatar || "https://i.pravatar.cc/100?img=1"),
            timestamp: message.timestamp || message.createdAt,
            type: message.messageType || "text"
          };
          addMessage(contact.id, formattedMessage);
        });
      }
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    console.log('Send button pressed. Message:', message, 'RoomId:', roomId, 'UserId:', userId);
    ...
    if (!userId) {
      console.log('UserId not available, cannot send message');
      Alert.alert("Error", "User not authenticated. Please log in again.");
      return;
    }
    
    // If roomId is not available yet, try to create the room first
    if (!roomId) {
      console.log('RoomId not available, creating room first...');
      try {
        const reqOptions = {
          method: "POST",
          headers: {"Content-Type": "application/json"},
        ...
        // Now send the message with the new roomId
        await sendMessageToRoom(data.id);
      } catch (error) {
      ...
      // RoomId is available, send message directly
      await sendMessageToRoom(roomId);
    }
  };

  const sendMessageToRoom = async (currentRoomId) => {
